...
public class UserNioSocketApplication {
	...
	public static void main(String[] args) throws Exception {
		...
		IUserSocketServer myServer = new UserNioSocketServerImpl(port);
		myServer.startup();
		Thread.sleep(3000);

		IUserSocketClient myClient = new UserNioSocketClient("localhost", port);
		...
		List<TestUser> users = myClient.execute(request, new GenericUserResponseHandlerImpl<List<TestUser>>());
